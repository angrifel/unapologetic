.PHONY: git-rev-list-last-release-commit git-diff-stat-since-last-release git-log-since-last-release check-go check-godoc check-docker check-act install-godoc view-godoc-locally install-git-hooks docker-golangci-lint lint-nocache lint test run-github-action
...
git-diff-stat-since-last-release:
	@git diff --name-only $(shell ./release-management.sh last-release-rev)...HEAD
