...
var testBytes []byte  // test data; same as testString but as a s
...
func check(t *testing.T, testname string, buf *ReadSeekBuffer, s string) {
	bytes := buf.Bytes()
	str := buf.String()
...
func fillString(t *testing.T, testname string, buf *ReadSeekBuffer, s string, n int, fus string) string {
...
// The initial contents of buf corresponds to the string s;
// the result is the final contents of buf returned as a string.
func fillBytes(t *testing.T, testname string, buf *ReadSeekBuffer, s string, n int, fub []byte) string {
	check(t, testname+" (fill 1)", buf, s)
	for ; n > 0; n-- {
		m, err := buf.Write(fub)
		if m != len(fub) {
...
func TestNewReadSeekableBuffer(t *testing.T) {
	buf := NewReadSeekBuffer(testBytes)
...
func TestNewReadSeekableBufferString(t *testing.T) {
	buf := NewReadSeekBufferString(testString)
	check(t, "NewBufferString", buf, testString)
}

// Empty buf through repeated reads into fub.
// The initial contents of buf corresponds to the string s.
func empty(t *testing.T, testname string, buf *ReadSeekBuffer, s string, fub []byte) {
...
func TestCapWithPreallocatedSlice(t *testing.T) {
	buf := NewReadSeekBuffer(make([]byte, 10))
	n := buf.Cap()
	if n != 10 {
...
func TestCapWithSliceAndWrittenData(t *testing.T) {
	buf := NewReadSeekBuffer(make([]byte, 0, 10))
	buf.Write([]byte("test"))
	n := buf.Cap()
	if n != 10 {
		t.Errorf("expected 10, got %d", n)
...
func TestReadFromNegativeReader(t *testing.T) {
	...
	defer func() {
		...
		case nil:
			t.Fatal("ioaux.ReadSeekBuffer.ReadFrom didn't panic")
		case error:
			// this is the error string of errNegativeRead
			wantError := "ioaux.ReadSeekBuffer: reader returned negative count from Read"
...
func TestReadBytes(t *testing.T) {
	for _, test := range readBytesTests {
		buf := NewReadSeekBufferString(test.buffer)
		var err error
...
func TestGrowOverflow(t *testing.T) {
	...
	buf := NewReadSeekBuffer(make([]byte, 1))
...
func TestBufferGrowth(t *testing.T) {
	...
	cap1 := b.Cap()
	// (*ReadSeekableBuffer).grow allows for 2x capacity slop before sliding,
...
func BenchmarkWriteByte(b *testing.B) {
	const n = 4 << 10
	b.SetBytes(n)
	buf := NewReadSeekBuffer(make([]byte, n))
	for i := 0; i < b.N; i++ {
		buf.Reset()
		for i := 0; i < n; i++ {
			buf.WriteByte('x')
...
func BenchmarkWriteRune(b *testing.B) {
	...
	buf := NewReadSeekBuffer(make([]byte, n*utf8.UTFMax))
	for i := 0; i < b.N; i++ {
		buf.Reset()
		for i := 0; i < n; i++ {
			buf.WriteRune(r)
...
func TestReader(t *testing.T) {
	r := NewReadSeekBuffer([]byte("0123456789"))
	tests := []struct {
		off     int64
		seek    int
		n       int
		...
		{seek: io.SeekStart, off: -1, seekerr: "ioaux.ReadSeekBuffer.Seek: negative position"},
		{seek: io.SeekStart, off: 1 << 33, n: 1, wantpos: 1 << 33, readerr: io.EOF},
		{seek: io.SeekCurrent, off: 1, n: 1, wantpos: 1<<33 + 1, readerr: io.EOF},
...
func TestReaderAt(t *testing.T) {
	...
	tests := []struct {
		...
		{11, 10, "", io.EOF},
		{0, 0, "", nil},
		{-1, 0, "", "ioaux.ReadSeekBuffer.ReadAt: negative offset"},
	}
	for i, tt := range tests {
		b := make([]byte, tt.n)
...
func TestReaderWriteTo(t *testing.T) {
	for i := 0; i < 30; i += 3 {
		...
		s := testString[:l]
		r := NewReadSeekBuffer(testBytes[:l])
		var b bytes.Buffer
		n, err := r.WriteTo(&b)
...
var UnreadRuneErrorTests = []struct {
	name string
	f    func(*ReadSeekBuffer)
}{
	{"Read", func(r *ReadSeekBuffer) { r.Read([]byte{0}) }},
	{"ReadByte", func(r *ReadSeekBuffer) { r.ReadByte() }},
	{"UnreadRune", func(r *ReadSeekBuffer) { r.UnreadRune() }},
	{"Seek", func(r *ReadSeekBuffer) { r.Seek(0, io.SeekCurrent) }},
	{"WriteTo", func(r *ReadSeekBuffer) { r.WriteTo(&bytes.Buffer{}) }},
}

func TestUnreadRuneError(t *testing.T) {
	for _, tt := range UnreadRuneErrorTests {
		reader := NewReadSeekBuffer([]byte("0123456789"))
...
func TestReaderDoubleUnreadRune(t *testing.T) {
	buf := NewReadSeekBuffer([]byte("groucho"))
	if _, _, err := buf.ReadRune(); err != nil {
		// should not happen
		t.Fatal(err)
	}
...
func TestReaderCopyNothing(t *testing.T) {
	...
	with.n, with.err = io.Copy(discard, NewReadSeekBuffer(nil))
	withOut.n, withOut.err = io.Copy(discard, justReader{NewReadSeekBuffer(nil)})
	if with != withOut {
...
func TestReaderReset(t *testing.T) {
	r := NewReadSeekBuffer([]byte("世界"))
	if _, _, err := r.ReadRune(); err != nil {
		t.Errorf("ReadRune: unexpected error: %v", err)
	}

	const want = "abcdef"
	r = NewReadSeekBuffer([]byte(want))
...
func TestReaderZero(t *testing.T) {
	if l := (&ReadSeekBuffer{}).Len(); l != 0 {
		t.Errorf("Len: got %d, want 0", l)
	}

	if n, err := (&ReadSeekBuffer{}).Read(nil); n != 0 || err != nil {
		t.Errorf("Read: got %d, %v; want 0, nil", n, err)
	}

	if n, err := (&ReadSeekBuffer{}).ReadAt(nil, 11); n != 0 || err != io.EOF {
		t.Errorf("ReadAt: got %d, %v; want 0, io.EOF", n, err)
	}

	if b, err := (&ReadSeekBuffer{}).ReadByte(); b != 0 || err != io.EOF {
		t.Errorf("ReadByte: got %d, %v; want 0, io.EOF", b, err)
	}

	if ch, size, err := (&ReadSeekBuffer{}).ReadRune(); ch != 0 || size != 0 || err != io.EOF {
		t.Errorf("ReadRune: got %d, %d, %v; want 0, 0, io.EOF", ch, size, err)
	}

	if offset, err := (&ReadSeekBuffer{}).Seek(11, io.SeekStart); offset != 11 || err != nil {
		t.Errorf("Seek: got %d, %v; want 11, nil", offset, err)
	}

	if s := (&ReadSeekBuffer{}).Size(); s != 0 {
		t.Errorf("Size: got %d, want 0", s)
	}

	if (&ReadSeekBuffer{}).UnreadByte() == nil {
		t.Errorf("UnreadByte: got nil, want error")
	}

	if (&ReadSeekBuffer{}).UnreadRune() == nil {
		t.Errorf("UnreadRune: got nil, want error")
	}

	if n, err := (&ReadSeekBuffer{}).WriteTo(io.Discard); n != 0 || err != nil {
